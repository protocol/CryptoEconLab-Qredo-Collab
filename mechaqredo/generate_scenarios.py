...
def generate_n_trx_scenario(params_dict: dict, scenario: str, sim_length: int) -> ParamsDict:
    """
    ...
    rates = {
        'very bad': -0.75 * N_trx_constant / sim_length,
        'bad': -0.25 * N_trx_constant / sim_length,
        'base': N_trx_constant,
        'good': N_trx_constant / sim_length,
        'very good': 2 * N_trx_constant / sim_length
    }
...
def generate_arrival_rate_scenario(params_dict: dict, scenario: str,
                                   VALIDATOR_JOINING_RATES: dict = {'very bad': 1 / 365, 'bad': 1 / 180, 'base': 1 / 90,
                                                                   'good': 1 / 30, 'very good': 1 / 30}) -> ParamsDict:
    """
...
def generate_price_or_fee_scenario(params_dict: dict, model_name: str, scenario: str, max_drift: float = 0.8,
                                   volatility: float = 0.4) -> ParamsDict:
    """
    ...
    Args:
        ...
        max_drift (float): The maximum drift value for the token price model.
    ...
    """
    assert scenario in SCENARIOS, f"Invalid scenario. Expected one of: {SCENARIOS}"
    assert model_name in ['token_price_model', 'service_fees_model'], "Invalid model_name. Expected one of: ['token_price_model', 'service_fees_model']"
...
def generate_full_scenario(params_dict: dict, scenarios_dict: dict,forecast_length:int) -> ParamsDict:
    """
    ...
    params_dict = generate_price_or_fee_scenario(params_dict=params_dict,
                                                 scenario=scenarios_dict['service_fee_model'],
                                                 model_name='service_fees_model')
    params_dict = generate_price_or_fee_scenario(params_dict=params_dict, scenario=scenarios_dict['price_model'],
                                                 model_name='token_price_model')
    params_dict = generate_arrival_rate_scenario(params_dict=params_dict, scenario=scenarios_dict['n_validators'])
    params_dict = generate_n_trx_scenario(params_dict=params_dict, scenario=scenarios_dict['n_trx'],
...
if __name__=='__main__':
    ...
    for s in SCENARIOS:
        ...
        sd={'service_fee_model':s,
            'price_model':s,
            'n_validators':s,
            'n_trx':s}
        
        params_dict=generate_full_scenario(params_dict=params_dict,
                                           scenarios_dict=sd,
        ...
        df['token_price'].plot(); plt.title('Price')
        plt.subplot(2,2,2)
        df['service_fees'].plot(); plt.title('service fee')
        plt.subplot(2,2,3)
        df['n_validators'].plot(); plt.title('N validators')
        plt.subplot(2,2,4)
        df['n_txs'].plot(); plt.title('N trx');
        plt.tight_layout()
        plt.show()
